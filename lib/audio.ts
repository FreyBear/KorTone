...
async function getInstrument(): Promise<PlayableInstrument> {
  ...
  if (!instrumentPromises[mode]) {
    instrumentPromises[mode] = createInstrument(mode).catch((error) => {
      delete instrumentPromises[mode];
      throw error;
    });
  }
...
export async function preloadCurrentInstrument(): Promise<void> {
  await getInstrument();
}

export function installAudioWarmupOnFirstGesture(): () => void {
  if (typeof window === 'undefined') {
    return () => undefined;
  }

  const warmup = () => {
    void primeAudioContext().catch(() => undefined);
    void preloadCurrentInstrument().catch(() => undefined);
  };

  const options: AddEventListenerOptions = { once: true, passive: true };
  window.addEventListener('pointerdown', warmup, options);
  window.addEventListener('touchstart', warmup, options);
